              ...
              <div style="display: flex; gap: 12px; margin-top: 14px;">
                <div class="netflix-form-group" style="flex: 1; margin-bottom: 0;">
                  ...
                  <select id="dm-rating-edit" class="netflix-form-select" style="background:#141414; color:white; border: 1px solid rgba(255,255,255,0.15); border-radius:4px; padding: 10px; font-size:12px; width:100%;">
                    <option value="U" ${m.rating === 'U' || m.rating === 'G' ? 'selected' : ''}>U (Universal - All Ages)</option>
                    <option value="U/A 7+" ${m.rating === 'U/A 7+' || m.rating === 'PG' ? 'selected' : ''}>U/A 7+ (Parental Guidance under 7)</option>
                    <option value="U/A 13+" ${m.rating === 'U/A 13+' || m.rating === 'PG-13' || m.rating === 'TV-14' ? 'selected' : ''}>U/A 13+ (Parental Guidance under 13)</option>
                    <option value="U/A 16+" ${m.rating === 'U/A 16+' ? 'selected' : ''}>U/A 16+ (Parental Guidance under 16)</option>
                    <option value="U/A 18+" ${m.rating === 'U/A 18+' || m.rating === 'R' || m.rating === 'NC-17' || m.rating === '18+' || m.rating === 'TV-MA' ? 'selected' : ''}>U/A 18+ (Adults Only)</option>
                  </select>
                </div>
                <div class="netflix-form-group" style="flex: 1; margin-bottom: 0;">
...
1. NATURAL REALISM & MAXIMUM AUTHENTICITY: Keep the generated image 100% realistic, warm, and authentic. It MUST look like a real, high-quality photograph or live-action movie frame rather than digital fantasy illustration or artificial, smooth-looking CGI. Fully preserve the core subjects, facial features, expressions, and primary background details from the original reference photo without over-modifying or distorting them.
...
3. NETFLIX BRANDING: Tastefully overlay a small, official red Netflix "N" letter icon (not the full "Netflix" name) in the top-left corner of the thumbnail, matching the exact size, subtle styling, and placement of professional Netflix series covers.
4. CUSTOM TITLE GRAPHIC OVERLAY: Tastefully overlay the title "${title}" anywhere on the image where it fits best and is highly legible. Rather than a basic generic font, render the title in a custom, highly stylized movie title logo graphic (such as a gorgeous custom serif, modern bold display, elegant artistic script, or stylized film lettering depending on the genre and mood). Style the title text beautifully as integrated key art with subtle drop shadows or a soft back-glow for perfect readability.
